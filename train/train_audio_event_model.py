#!/usr/bin/env python3
"""Train a small audio event model for TFLite Micro style deployment.
...
The silence class is generated automatically. Audio is converted to 16 kHz,
cropped or padded to one second, converted to a 49 x 40 log-mel feature image,
and classified by a small DS-CNN model.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a small local audio event detection model.",
    ...
    parser.add_argument(
        "--focal-loss",
        action="store_true",
        help="Use Focal Loss instead of standard cross-entropy (focuses on hard samples).",
    )
    parser.add_argument(
        "--focal-loss-gamma",
        type=float,
        default=2.0,
        help="Gamma parameter for Focal Loss. Larger values focus more on hard samples.",
    )
    parser.add_argument(
        "--delta-features",
        action="store_true",
...
class DatasetIndex:
    ...
    def _scan(self) -> None:
        """Scan dataset: prefer pre-split train/val/test dirs, fall back to random split."""
    ...
    def _has_presplit(self) -> bool:
        """Check if train/val/test subdirectories exist with content."""
    ...
    def _scan_from_presplit(self) -> None:
        """Read samples directly from train/val/test/{class} directories."""
    ...
    def _scan_with_random_split(self) -> None:
        """Fallback: scan flat class dirs and randomly split (legacy behavior)."""
    ...
    def _append_generated_silence(self) -> None:
        ...
        for split_name, samples in self.samples_by_split.items():
            ...
            if real_count == 0:
                ...
                silence_count = max(1, int(round(real_count * self.args.silence_ratio)))
            samples.extend(
...
class FeatureExtractor:
    def __init__(self, args: argparse.Namespace, dataset: DatasetIndex):
        ...
        self.background_paths = [
            sample.path
            for sample in dataset.samples_by_split["train"]
            if sample.class_name == dataset.background_class
            and not sample.path.startswith(SILENCE_TOKEN)
        ]
    ...
    def _crop_or_pad(audio: np.ndarray, training: bool) -> np.ndarray:
        ...
        if len(audio) < target_len:
            ...
            if training:
                start = random.randint(0, target_len - len(audio))
            else:
                start = 0
    ...
    def make_feature(self, audio: np.ndarray, training: bool) -> np.ndarray:
        ...
        if self.args.pcen:
            ...
            # Rescale PCEN output to roughly [0, 26] for quantized input compatibility
            # PCEN raw range with defaults: ≈ [-delta^root, 10]; rescale to fill [0, 26]
            feature = (feature + 1.2) * 8.0
            feature = tf.clip_by_value(tf.convert_to_tensor(feature), QUANT_INPUT_MIN, QUANT_INPUT_MAX)
        else:
        ...
        if self.args.delta_features:
            # Compute delta (frame difference) and delta² (second-order difference)
        ...
        # Guarantee consistent size: STFT(pad_end=False) may yield fewer frames
        # when audio is not exactly 16000 samples after resampling.
...
def make_dataset(
    ...
    if training:
        dataset = dataset.shuffle(max(32, len(samples) * 2), reshuffle_each_iteration=True)
...
def build_model(num_classes: int, model_size: str, num_channels: int = 1,
                use_se: bool = False) -> tf.keras.Model:
    configs = {
        "micro": (4, 8, 8),
        "tiny": (8, 12, 16),
        "small": (12, 16, 24),
    }
    filters = configs[model_size]
    ...
    x = conv_bn_relu(x, filters[0], kernel_size=5, strides=2, name="stem")
    x = ds_conv_block(x, filters[1], strides=1, name="ds1")
    if use_se:
        x = se_block(x, name="se1")
    x = ds_conv_block(x, filters[2], strides=1, name="ds2")
    if use_se:
    ...
    return tf.keras.Model(inputs=inputs, outputs=outputs, name=f"audio_event_{model_size}")
...
def se_block(x: tf.Tensor, ratio: int = 4, name: str = "se") -> tf.Tensor:
    ...
    s = tf.keras.layers.Dense(max(1, channels // ratio), activation="relu",
                               name=f"{name}_fc1")(s)
...
def train(args: argparse.Namespace) -> None:
    ...
    if not args.no_plots:
        maybe_write_feature_samples(output_dir / "feature_samples.png", extractor, dataset_index)
    ...
    train_ds = make_dataset(train_samples, extractor, args.batch_size, training=True,
                            flat_feature_size=flat_feat_size)
    val_ds = make_dataset(val_samples, extractor, args.batch_size, training=False,
                          flat_feature_size=flat_feat_size)
    test_ds = make_dataset(test_samples, extractor, args.batch_size, training=False,
                           flat_feature_size=flat_feat_size)

    model = build_model(
        len(dataset_index.class_names), args.model_size, num_channels=num_channels,
        use_se=args.se_attention)
    model.summary()
    if args.focal_loss:
    ...
    class_weight = compute_class_weight(train_samples, len(dataset_index.class_names))
    history = model.fit(
        train_ds,
    ...
    if not args.no_plots:
        maybe_write_training_curves(output_dir / "training_curves.png", history.history)
    ...
    if tflite_path.exists() and test_samples:
        ...
        tflite_metrics = evaluate_tflite_int8(tflite_path, test_ds, dataset_index.class_names)
        write_json(output_dir / "metrics_tflite_int8.json", tflite_metrics)
        print(f"  int8 accuracy: {tflite_metrics['test_accuracy']:.4f}  "
...
def evaluate_model(
    ...
    if not args.no_plots:
        maybe_write_confusion_plot(output_dir / "confusion_matrix.png", cm, class_names)
        if y_score and len(class_names) > 1:
            maybe_write_roc_curves(output_dir / "roc_curves.png", y_true, y_score, class_names)

    non_event_name_set = set(non_event_names)
    event_indices = [i for i, name in enumerate(class_names) if name not in non_event_name_set]
    non_event_indices = [i for i, name in enumerate(class_names) if name in non_event_name_set]
    false_alarm_count = sum(
        1 for truth, pred in zip(y_true, y_pred) if truth in non_event_indices and pred in event_indices
    )
    non_event_count = sum(1 for truth in y_true if truth in non_event_indices)
    miss_count = sum(
        1 for truth, pred in zip(y_true, y_pred) if truth in event_indices and pred not in event_indices
    )
    ...
    print(json.dumps({k: metrics[k] for k in ("test_accuracy", "false_alarm_rate", "miss_rate")}, indent=2))
    return metrics
...
def write_threshold_sweep(
    ...
    for cls_idx, cls_name in enumerate(class_names):
        ...
        for thresh in thresholds:
            # Per-class: event criterion = probability[cls] >= thresh
    ...
    overall_sweep = []
    for thresh in thresholds:
        pred_event = [max(s[i] for i in event_indices) >= thresh for s in y_score]
        tp = sum(int(t in event_indices and p) for t, p in zip(y_true, pred_event))
        fp = sum(int(t in non_event_indices and p) for t, p in zip(y_true, pred_event))
        fn = sum(int(t in event_indices and not p) for t, p in zip(y_true, pred_event))
        event_count = sum(1 for t in y_true if t in event_indices)
        non_event_count = sum(1 for t in y_true if t in non_event_indices)
        overall_sweep.append({
            "threshold": round(thresh, 2),
            "false_alarm_rate": round(fp / non_event_count, 4) if non_event_count else 0,
            "miss_rate": round(fn / event_count, 4) if event_count else 0,
        })
...
def _consecutive_trigger_sweep(
    ...
    for thresh in thresholds:
        ...
        tp = sum(int(t in event_indices and triggered[i]) for i, t in enumerate(y_true))
        fp = sum(int(t in non_event_indices and triggered[i]) for i, t in enumerate(y_true))
        fn = sum(int(t in event_indices and not triggered[i]) for i, t in enumerate(y_true))
        event_count = sum(1 for t in y_true if t in event_indices)
        non_event_count = sum(1 for t in y_true if t in non_event_indices)
        results.append({
            "threshold": round(thresh, 2),
...
def evaluate_tflite_int8(
    ...
    for features, labels in test_ds:
        for i in range(features.shape[0]):
            ...
            interpreter.set_tensor(input_details[0]["index"], np.expand_dims(feat_int8, 0))
            interpreter.invoke()
...
def write_metadata(
    ...
    metadata = {
        ...
        "flat_feature_size": FEATURE_FRAME_COUNT * FEATURE_BIN_COUNT * (3 if args.delta_features else 1),
        ...
        "trigger_recommendation": "alarm after probability >= 0.75 for 2 consecutive inferences",
        "metrics": {
            key: metrics[key]
...
def write_training_summary(path: Path, history: dict) -> None:
    serializable = {key: [float(item) for item in value] for key, value in history.items()}
    write_json(path, serializable)


def write_confusion_csv(path: Path, matrix: np.ndarray, class_names: list[str]) -> None:
...
def maybe_write_training_curves(
    ...
    """Write loss and accuracy curves as a PNG."""
    try:
...
def maybe_write_roc_curves(
    ...
    """Write per-class ROC curves as a PNG."""
    try:
        import matplotlib.pyplot as plt
    except ImportError:
...
def maybe_write_feature_samples(
    ...
    """Write sample mel-spectrogram features for each class as a PNG."""
    try:
        import matplotlib.pyplot as plt
    except ImportError:
    ...
    for ax, cls in zip(axes, class_names):
        samples = [s for s in dataset_index.samples_by_split["train"] if s.class_name == cls]
        if not samples:
        ...
        # Reshape flat feature; for multi-channel only show static Mel (channel 0)
        n = len(feature)
        base = FEATURE_FRAME_COUNT * FEATURE_BIN_COUNT
        channels = n // base
        feat_3d = feature.reshape(FEATURE_FRAME_COUNT, FEATURE_BIN_COUNT, channels)
        feat_img = feat_3d[:, :, 0]  # Show static Mel only for visualization
