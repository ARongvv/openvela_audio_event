...
int esp32s3_bringup(void)
{
...
#ifdef CONFIG_ESP32S3_BOX_AUDIO
  /* Configure the microphone ADC before exposing the I2S capture device. */

  ret = esp32s3_es7210_initialize(ES7210_I2C_PORT, ES7210_I2C_ADDR,
                                  ES7210_I2C_FREQUENCY);
  if (ret < 0)
    {
  ...
  /* LCD must initialise before the touchscreen because both share GPIO 48
   * as a reset line.  The LCD driver performs the hardware reset which
   * also resets the GT911.
   */
