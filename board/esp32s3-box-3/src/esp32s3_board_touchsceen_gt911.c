...
/* GT911 I2C addresses.
 * The GT911 samples the INT pin during reset release to select its
 * address: INT=LOW -> GT911_ADDR (0x5d), INT=HIGH -> GT911_ADDR_BACKUP
 * (0x24).  We try both at runtime so the driver works regardless of the
 * pin state at reset time.
 */

#define GT911_ADDR            TOUCHSCEEN_ADDR
#define GT911_ADDR_BACKUP     0x24
...
struct gt911_dev_s
{
  ...
  uint8_t i2c_addr;                         /* Discovered I2C address */
...
/****************************************************************************
 * Name: gt911_verify_communication
 *
 * Description:
 *   Read GT911 product ID register to verify I2C communication at the
 *   given address.  The probe address is stored in dev->i2c_addr on
 *   success so that subsequent reads/writes use the discovered address.
 *
 * Input Parameters:
 *   dev   - GT911 object pointer
 *   addr  - I2C address to probe
 *
 * Returned Value:
...
static int gt911_verify_communication(struct gt911_dev_s *dev)
{
  int ret;
  uint8_t id[4];

  ret = gt911_read_reg(dev, GT911_PRODUCT_ID_REG, id, sizeof(id));
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: GT911 not responding on I2C addr 0x%02x\n",
             dev->i2c_addr);
      return ret;
    }

  syslog(LOG_INFO, "GT911 Product ID: %c%c%c%c (addr=0x%02x)\n",
         id[0], id[1], id[2], id[3], dev->i2c_addr);
 ...
 *
 * Description:
 *   Probe the GT911 at both possible I2C addresses.  The GT911 selects
 *   its address by sampling the INT pin during reset release:
 *     INT=LOW  -> GT911_ADDR      (0x5d)
 *     INT=HIGH -> GT911_ADDR_BACKUP (0x24)
 *
 *   This function also restores the INT pin (GPIO 3) to INPUT_PULLUP
 *   for future interrupt use.
 *
 * Input Parameters:
 *   dev - GT911 object pointer
 *
 * Returned Value:
 *   Zero (OK) on success; -ENODEV if neither address responds.
 *
 ****************************************************************************/

static int gt911_probe(struct gt911_dev_s *dev)
{
  int ret;

  /* Restore INT pin to input for I2C communication */

  esp32s3_configgpio(TOUCHSCEEN_INT, INPUT_PULLUP);

  syslog(LOG_INFO, "GT911: probing I2C bus %d, trying addr 0x%02x...\n",
         TOUCHSCEEN_I2C, GT911_ADDR);

  /* Try the primary address first (INT was LOW during reset) */

  dev->i2c_addr = GT911_ADDR;
  ret = gt911_verify_communication(dev);
  if (ret == OK)
    {
      syslog(LOG_INFO, "GT911: found at primary addr 0x%02x\n",
             dev->i2c_addr);
      return OK;
    }

  syslog(LOG_INFO, "GT911: primary addr 0x%02x failed (%d), "
         "trying backup 0x%02x...\n", GT911_ADDR, ret, GT911_ADDR_BACKUP);

  /* Try the backup address (INT was HIGH during reset) */

  dev->i2c_addr = GT911_ADDR_BACKUP;
  ret = gt911_verify_communication(dev);
  if (ret == OK)
    {
      syslog(LOG_INFO, "GT911: found at backup addr 0x%02x\n",
             dev->i2c_addr);
      return OK;
    }

  syslog(LOG_ERR, "ERROR: GT911 not found at 0x%02x (%d) or 0x%02x (%d)\n",
         GT911_ADDR, ret, GT911_ADDR_BACKUP, ret);
  return -ENODEV;
}
...
int board_touchscreen_initialize(void)
{
  ...
  syslog(LOG_INFO, "board_touchscreen_initialize: I2C bus=%d, "
         "SCL=GPIO%d, SDA=GPIO%d\n",
         TOUCHSCEEN_I2C, CONFIG_ESP32S3_I2C0_SCLPIN,
         CONFIG_ESP32S3_I2C0_SDAPIN);
  ...
  syslog(LOG_INFO, "board_touchscreen_initialize: I2C bus initialized, "
         "probing GT911...\n");
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: GT911 probe failed: %d\n", ret);
      return ret;
    }
